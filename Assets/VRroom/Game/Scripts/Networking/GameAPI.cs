...
namespace VRroom.Game.Networking {
	...
	public class GameAPI : BaseAPI<GameAPI> {
		...
		private static async Task<string> Login(string[] args) {
			...
			return response.Success ? $"Successfully Logged in as {args[0]}" : $"Failed to Login: {response.Error}";
		}
		...
		private static async Task<string> CreateAccount(string[] args) {
			...
			return response.Success ? "Successfully created account" : $"Failed to Create Account: {response.Error}";
		}
	}
}